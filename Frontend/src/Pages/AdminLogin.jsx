  ...
  const handleAdminLogin = (e) => {
    ...
    mutate(data, {
      onSuccess: (responseData) => {
        console.log("admin login data", responseData);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-900 text-white">
      <div className="bg-gray-800 p-8 rounded-lg shadow-lg w-full max-w-md">
        <h1 className="text-2xl font-bold mb-6 text-center">Login as Seller</h1>

        {isError && (
          <p className="bg-red-600 text-white px-4 py-2 mb-4 rounded">
            {error?.response?.data?.error || "Error in Login"}
          </p>
        )}
